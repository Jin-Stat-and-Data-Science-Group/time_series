...
#习题6.2
#1、用单位根检验，考察模型平稳性
dat<- read.table('习题6.2数据.txt',fill=T)
grain <- dat[2:5,]
grain <- as.numeric(t(as.matrix(grain)))
grain <- ts(na.omit(grain)) #转为时间序列
rain <- dat[7:10,]
rain <- as.numeric(t(as.matrix(rain)))
rain <- ts(na.omit(rain)) #转为时间序列
c1=min(grain,rain)
c2=max(grain,rain)
plot(grain,ylim=c(c1,c2))
lines(rain,col="red")
#对谷物产出进行ADF检验
library(fBasics)
library(fUnitRoots)
...
##输出结果显示，残差序列平稳，谷物产出量与降雨量具有协整关系，回归模型为：grain=23.5521+0.7755*rain

#习题6.4
dat1<- read.table('习题6.4数据.txt',header = T,sep = "\t",encoding = "UTF-8",skipNul = T)
...
##检验结果显示，出口额对数差分序列是无常数均值的平稳序列，该平稳序列1阶自相关（ADF的P值=0.01215）
for (i in 1:3) print(adfTest(dif.lny,lag=i,type='nc')) 
##检验结果显示，进口额对数差分序列是无常数均值的平稳序列，该平稳序列1阶自相关（ADF的P值=0.01）
