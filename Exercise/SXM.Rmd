...
拒绝原假设,说明拟合arma(2,2)后的残差并不是正态分布
...
完整拟合模型为
$$\left\{\begin{array}{l}
{ r_t=0.0015-0.0985r_{t-1}-0.9167r_{t-2}+a_t+0.1218a_{t-1}+0.9013a_{t-2}}\\
{a_{t}=\sigma_{t} \varepsilon _{t}} \\
{\sigma_{t}^2=0.9728 \sigma_{t-1}^2+0.02116 a_{t-1}^{2}}
\end{array}\right.$$
...
mm=predict(rfit,3) #均值的三个预测值
RMeasure(1.525907e-04	,0.02665890	)
```

由结果可知，在T=2020/1/2时刻，超前1步预测的收益率均值`r mm$meanForecast[1]`，波动率为`r mm$meanError[1]`，相应的风险值为
$$\left\{\begin{array}{l}
VaR_{0.95}=0.04400258,ES_{0.95}=0.05514225\\
VaR_{0.99}=0.06217047,ES_{0.99}=0.07120427\\
VaR_{0.999}=0.08253478,ES_{0.999}=0.08956207
\end{array}\right.$$
