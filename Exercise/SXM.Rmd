...
#从雅虎财经中获得百度公司的数据
#getSymbols('BIDU') 
#write.csv(BIDU,"./data/BIDU.csv")
BIDU <- read.csv("./data/BIDU.csv")
dim(BIDU) #查看数据维度3273条，6个变量
head(BIDU) 
tail(BIDU) #2007-01-03至2020-01-02数据
#计算日对数收益率
options(digits = 4)
...
从收益率数据特征可知，对数收益率的偏度为-0.022980，峰度为8.248098，而正态分布的偏度和峰度分别为0和3，故对数收益率序列具有左偏性和尖峰性，初步判断不服从正态分布。
