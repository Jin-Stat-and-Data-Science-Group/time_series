...
用延迟1到d期的历史数据 ${\{x_{t-1}}\}、{\{x_{t-2}}\}\cdots {\{x_{t-d}}\}$ 为自变量来解释当期序列值 ${\{x_t}\}$ 变动状况，差分序列 ${\nabla ^d}{x_t}$ 量的是 ${\{x_t}\}$ 阶自回归过程中产生的随机误差的大小。
...
ARIMA(p,d,q)模型共有p+d个特征根，其中p个在单位圆内，d个在单位圆上。所以当 $d \ne 0$ ，ARIMA(p,d,q)模型非平稳。

例：拟合随机游走序列ARIMA(0,1,0), $x_t=x_{t-1}+{\varepsilon _t},{\varepsilon
_t}\sim NID(0,100)$ 
```{r}
...
l期的预测值 ${\hat x_t}(l)$ $${\hat x_t}(l)={\psi _l}{\varepsilon _t} + {\psi _{l + 1}}{\varepsilon _{t - 1}} +  \cdots$$

l期的预测误差 ${e_t}(l)$ \[\begin{array}{l}
...
如果该模型中有部分自相关系数 ${\phi _j},1 \le j < p$ 部分移动平滑系数 ${\theta _k},1 \le k<q$ 零，即原模型中有部分系数省缺了，那么该模型称为疏系数模型。

如果只是自相关部分有省缺系数，那么该疏系数模型可以简记为 $ARIMA(({p_1}, \cdots ,{p_m}),d,q)$ 
- ${p_1}, \cdots ,{p_m}$ 非零自相关系数的阶数

如果只是移动平滑部分有省缺系数，那么该疏系数模型可以简记为 $ARIMA(p,d,({q_1}, \cdots ,{q_n}))$ 
- ${q_1}, \cdots ,{q_n}$ 非零移动平均系数的阶数
...
（2）自变量为历史观察值 ${x_{t-1},x_{t-2},\cdots,x_{t-k}}$ \[{T_t} = {\beta _0} + {\beta _1} \cdot {x_{t - 1}} +  \cdots  + {\beta _k} \cdot {x_{t - k}} + {\varepsilon _t}\]
...
# 方差齐性变换
## 使用场合
- 序列显示出显著的异方差性，且方差与均值之间具有某种函数关系$$\sigma _t^2 = h({\mu _t}) $ $其中:$ (.)$是某个已知函数


## 处理思路
- 尝试寻找一个转换函数 $g(.)$ 使得经转换后的变量满足方差齐性
$ $Var[g({x_t})] = {\sigma ^2}$  $
- 转换函数$ (x_t) $在$μ_t$ 作一阶泰勒展开
...
- 假设条件
\[\begin{array}{l}
{H_0}: \text{残差平方序列纯随机（方差齐性）} \leftrightarrow {H_1}:\text{残差平方序列自相关（方差非齐）}\\
...
或者，当Q(q)检验统计量的P值小于显著性水平 $\alpha$ ，拒绝原假设，认为该序列方差非齐且具有自相关关系。
...
- 检验统计量
$$
LM(q)=\frac{(S S T-S S E) / q}{S S E /(T-2 q-1)}
$$
- 检验结果：
...
\end{array} \right.\]
式中， $f(t,{x_{t - 1}},{x_{t - 2}}, \cdots )$  $\left\{ {{x_t}} \right\}$ 确定性信息拟合模型； $\epsilon_{t} \stackrel{\mathrm{i.l.d}}{\sim} N\left(0, \sigma^{2}\right)$ 这个模型简记为GARCH(p,q).
- GARCH模型实质
...
### AR-GARCH模型
对序列 $\left\{ {{\varepsilon _t}} \right\}$ 合GARCH模型有一个基本要求： $\left\{ {{\varepsilon _t}} \right\}$ 为零均值，纯随机、异方差序列。有时回归函数 $f(t,{x_{t - 1}},{x_{t - 2}}, \cdots )$ 不能充分提取原序列中的相关信息， $\left\{ {{\varepsilon _t}} \right\}$ 能具有自相关性，而不是纯随机的。这时需要对 $\left\{ {{\varepsilon _t}} \right\}$ 先拟合自回归模型，再考察自回归残差序列 $\left\{ {{\nu_t}} \right\}$ 方差齐性，如果 $\left\{ {{\nu_t}} \right\}$ 异方差，对它拟合GARCH模型。这样构造的模型称为AR-GARCH模型
...
\end{array} \right.\]
式中， $f(t,{x_{t - 1}},{x_{t - 2}}, \cdots )$  $\left\{ {{x_t}} \right\}$ 确定性信息拟合模型； $\epsilon_{t} \stackrel{\mathrm{i.l.d}}{\sim} N\left(0, \sigma^{2}\right)$ 
### GARCH模型拟合步骤
1. 回归拟合
2. 残差自相关检验
3. 异方差自相关性检验
...
- 以 模型ARCH（1）为例， $\varepsilon _t^2{\rm{ = }}\phi \varepsilon _{t{\rm{ - }}1}^2$ 无论 ${\varepsilon _{t{\rm{ - }}1}}$ 正是负，它对下一期的影响系数都是 $\phi$ 。这意味着无论上一期的投资是收益还是亏损，不影响投资人的下一期投资状况，这与实际情况不符。
...
2. EGARCH的改进
- $\ln ({h_t})$ 条件方差的对数，他可以是它可以是正数也可以是负数，所以后面表达式
