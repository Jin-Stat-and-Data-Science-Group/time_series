...
### 宽平稳和严平稳的关系
严格来讲，宽平稳推不出严平稳，只有当序列服从多元正态分布时，二阶平稳可以推出严平稳，换言之，宽平稳正泰事件序列一定是严平稳时间序列。对于非正态过程，就没有这个性质。严平稳也推不出宽平稳，只有存在二阶矩的严平稳序列才能保证一定是宽平稳序列。比如柯西分布的严平稳序列就不是宽平稳序列。
...
### 自相关图检验
自相关图是一个平面二维坐标悬垂线图，横坐标表示延迟时期数，另一个坐标表示自相关系数，悬垂线表示自相关系数的大小。平稳序列通常具有短期相关性。该性质用自相关系数来描述就是随着延迟期数的增加，平稳序列的自相关系数会很快地衰减向零
...
\end{array}\]
称序列$\left\{X_{t}\right\}$为纯随机序列，也称白噪声序列，简记为$WN(\mu,\sigma ^2)$。白噪声序列一定是平稳序列，而且是最简单的平稳序列。
