...
记$\zeta_{i}$(i=1,2,,...,p)为上式系数矩阵中p个列向量，$\eta$为式中等号右边自相关系数向量，即
$$\zeta_{i}=\left( {\begin{array}{*{20}{c}}
{{\rho _{i-1}}}\\
{{\rho _{i-2}}}\\
 \vdots \\
{{\rho _{k-i}}}
...
则有
$$\eta=\phi_1\zeta_1+\phi_2\zeta_2+...+\phi_p\zeta_p$$

因为AR(p)模型的限制条件之一是$\phi_p\neq0$,所以向量$\eta$一定可以表示成向量$\zeta_{i}(i=1,2,...,p)$的非零线性组合。
当k>p时，
...
显然$D_k$的前p个列向量正好就是$\zeta_{i}(i=1,2,...,p)$，而最后一个列向量正好就是
